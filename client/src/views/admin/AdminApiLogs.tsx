...
import AdminService, { ApiLogDto } from '@/services/AdminService'

const AdminApiLogs = () => {
    const [items, setItems] = useState<ApiLogDto[]>([])
    ...
    const load = async () => {
        try {
            setLoading(true)
            const res = await AdminService.listApiLogs({
                page,
                limit,
        ...
        <div className="min-h-[calc(100vh-120px)] bg-slate-50/60 p-6">
            <div className="mx-auto flex max-w-[1400px] flex-col gap-6">
                <div>
                    ...
                    <h3 className="text-2xl font-semibold text-slate-800">API Logs</h3>
                    <p className="text-sm text-slate-500">Monitor external service calls.</p>
                </div>
                            ...
                            <tbody>
                                {items.map((item) => (
                                    <tr key={item.id || item.legacyId} className="border-t">
                                        ...
                                        <td className="px-4 py-2 max-w-[420px] truncate">{item.endpoint}</td>
                                        <td className="px-4 py-2">{item.statusCode ?? '-'}</td>
