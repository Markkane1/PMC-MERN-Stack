...
} from '../BusinessEntityForm/types'
import {
    LicenseDetailForm,
    LicenseDetailFormSchema,
} from '../LicenseDetailForm'
...
const CustomerEdit = () => {
            ...
            <div>
                ...
                {data.map((item, index) => (
                    ...
                    {index + 1})
                        ...
                        <div>
                            <strong>{index + 1}) Assigned By:</strong>{' '}
                            {item.created_by} ({item.created_by_group})
                        </div>
                        <div>
        ...
        const response = AxiosBase.get(`/pmc/applicant-detail/${id}/`, {
            ...
            .then((response) => {
                ...
                const data_applicantDetail = {
                    ...
                    applicationassignment: formatJsonToJsx(
                        response.data.applicationassignment,
                    ),
                ...
                if (response.data.businessprofile) {
                    if (
                        response.data.businessprofile.entity_type === 'Company'
                    ) {
                        const dataBusinessProfile = {
                            applicant: response.data.id,
                            businessName:
                                response.data.businessprofile.business_name,
                            ...
                            city: response.data.businessprofile
                                .city_town_village,
                            postalAddress:
                                response.data.businessprofile.postal_address,
                            mobileNumber:
                                response.data.businessprofile.mobile_no,
                    ...
                    if ('Individual' === 'Individual') {
                        const dataBusinessProfile = {
                            ...
                            // email: response.data.businessprofile.email,
                            district:
                                response.data.businessprofile.district_name,
                    ...
                    updateBusinessEntity({
                        businessEntityType:
                            response.data.businessprofile.entity_type, //response.data.businessprofile.entity_type
                    })
                }

                // Handle Producer Data
                updateLicenseDetail({
                    licenseType: response.data.registration_for,
                })
                if (
                    response.data.producer &&
                    response.data.registration_for === 'Producer'
                ) {
                    const dataProducer = {
                        tracking_number:
                            response.data.producer.tracking_number || '',
                        registration_required_for:
                            response.data.producer.registration_required_for ||
                            [],
                        ...
                        plain_plastic_Sheets_for_food_wrapping:
                            response.data.producer
                                .plain_plastic_sheets_for_food_wrapping || [],
                        PackagingItems:
                            response.data.producer.packaging_items || [],
                        number_of_machines:
                            response.data.producer.number_of_machines || '',
                        total_capacity_value:
                            response.data.producer.total_capacity_value?.toString() ||
                            '',
                        date_of_setting_up:
                            response.data.producer.date_of_setting_up || '',
                        total_waste_generated_value:
                            response.data.producer.total_waste_generated_value?.toString() ||
                            '',
                        has_waste_storage_capacity:
                            response.data.producer.has_waste_storage_capacity ||
                            '',
                        waste_disposal_provision:
                            response.data.producer.waste_disposal_provision ||
                            '',
                        applicant: response.data.producer.applicant || '',
                        totalFeeAmount: response.data.applicantfees
                            ? response.data.applicantfees.reduce(
                                  (sum, fee) =>
                                      sum + parseFloat(fee.fee_amount || 0),
                                  0,
                              )
                            : 0,
                ...
                if (
                    ...
                    response.data.registration_for === 'Consumer'
                ) {
                    const dataConsumer = {
                        registration_required_for:
                            response.data.consumer.registration_required_for ||
                            [],
                        registration_required_for_other:
                            response.data.consumer
                                .registration_required_for_other || [],
                        ...
                        plain_plastic_Sheets_for_food_wrapping:
                            response.data.consumer
                                .plain_plastic_sheets_for_food_wrapping || [],
                        PackagingItems:
                            response.data.consumer.packaging_items || [],
                        consumption: response.data.consumer.consumption || '',
                        provision_waste_disposal_bins:
                            response.data.consumer
                                .provision_waste_disposal_bins || 'No',
                        no_of_waste_disposible_bins:
                            response.data.consumer
                                .no_of_waste_disposable_bins || '',
                ...
                if (
                    response.data.recycler &&
                    response.data.registration_for === 'Recycler'
                ) {
                    const dataRecycler = {
                        selectedCategories:
                            response.data.recycler.selected_categories || [],
                        plastic_waste_acquired_through:
                            response.data.recycler
                                .plastic_waste_acquired_through || [],
                        has_adequate_pollution_control_systems:
                            response.data.recycler
                                .has_adequate_pollution_control_systems || 'No',
                        pollution_control_details:
                            response.data.recycler.pollution_control_details ||
                            '',
                ...
                if (
                    ...
                    response.data.registration_for === 'Collector'
                ) {
                    const dataCollector = {
                        registration_required_for:
                            response.data.collector.registration_required_for ||
                            [],
                        registration_required_for_other:
                            response.data.collector
                                .registration_required_for_other || [],
                        ...
                        selectedCategoriesCollector:
                            response.data.collector.selected_categories || [],
                        total_capacity_value_collector:
                            response.data.collector.total_capacity_value || '',
                        number_of_vehicles:
                            response.data.collector.number_of_vehicles || '',
                        number_of_persons:
                            response.data.collector.number_of_persons || '',
    ...
    const handleBusinessEntityFormSubmit = async (
        values: BusinessEntityFormSchema,
    ) => {
        const formData = new FormData()
        ...
        if (completedSections.includes('businessDetailIndividual')) {
            updateBusinessDetailIndividual(
                values as BusinessDetailIndividualFields,
            )
    ...
    const handleLicenseDetailFormSubmit = async (
        ...
        console.log(
            'registration_required_for',
            values.registration_required_for,
        )

        // formData.append('registration_required_for', JSON.stringify(values.registration_required_for))
        formData.append(
            'is_carry_bags',
            values.registration_required_for.includes('Carry bags').toString(),
        )
        formData.append(
            'is_single_use_plastics',
            values.registration_required_for
                .includes('Single-use Plastics')
                .toString(),
        )
        formData.append(
            'is_plastic_packing',
            values.registration_required_for
                .includes('Plastic Packing')
                .toString(),
        )
        // Add non-file fields
        // formData.append('registration_required_for', JSON.stringify(values.registration_required_for));
        ...
        try {
            ...
            if (completedSections.includes('licenseDetailRecycler')) {
                updateLicenseDetailRecycler(
                    values as LicenseDetailFieldsRecycler,
                )
            }
    ...
    const handleSubmitManualFields = async () => {
        ...
        formData.append(
            'latitude',
            manualFields.latitude
                ? parseFloat(manualFields.latitude).toFixed(6)
                : '',
        )
        formData.append(
            'longitude',
            manualFields.longitude
                ? parseFloat(manualFields.longitude).toFixed(6)
                : '',
        )
    ...
    const submitApplication = async () => {
        ...
        setIsSubmiting(true)
        if (
            applicantDetail.assignedGroup2 !== '' &&
            applicantDetail.assignedGroup2 !== undefined
        ) {
            // Add non-file fields
            // const formData = new FormData();
    ...
    return (
        <>
                    ...
                    <div className="flex items-center justify-between px-8">
                        ...
                        <div className="flex items-center">
                            <Button
                                ...
                                icon={<TablerIcon name="trash" />}
                                onClick={handleDiscard}
                            >
                                Discard
                            </Button>
                            <Button
                                icon={<TablerIcon name="arrow-left" />}
                                className="ltr:mr-3 rtl:ml-3"
                                variant="solid"
                                type="button"
                                loading={isSubmiting}
                            ...
                            <Button
                                icon={<TablerIcon name="device-floppy" />}
                                variant="solid"
                                type="button"
                                loading={isSubmiting}
                            ...
                            >
                                ...
                                groupList[2]?.label === 'Download License'
            ...
            >
                <p>
                    There are unsaved changes on this step, Are you sure you
                    want to go back step?
                </p>
            </ConfirmDialog>
        </>
    )
