...
const CustomerEdit = () => {
                ...
                .then((response) => {
                    ...
                    const data_applicantDetail = {
                        ...
                        applicantDesignation:
                            response.data.applicant_designation,
                        ...
                        has_identity_document:
                            response.data.has_identity_document,
                        ...
                        is_downloaded_fee_challan:
                            response.data.is_downloaded_fee_challan,
                    }
                    ...
                    if (response.data.businessprofile) {
                        ...
                        if ('Individual' === 'Individual') {
                            const dataBusinessProfile = {
                                ...
                                // email: response.data.businessprofile.email,
                                district:
                                    response.data.businessprofile.district,
                                tehsil: response.data.businessprofile.tehsil,
                        ...
                        updateBusinessEntity({
                            businessEntityType:
                                response.data.businessprofile.entity_type,
                        })
                    }
                    ...
                    if (
                        ...
                        const dataConsumer = {
                            ...
                            consumption:
                                response.data.consumer.consumption || '',
        ...
        try {
            ...
            if (completedSections.includes('licenseDetailProducer')) {
                ...
                formData.append(
                    'registration_required_for_other',
                    JSON.stringify(
                        values.registration_required_for_other || [],
                    ),
                )
                formData.append(
                    'plain_plastic_sheets_for_food_wrapping',
                    JSON.stringify(
                        values.plain_plastic_Sheets_for_food_wrapping || [],
                    ),
                )
                formData.append(
                    'packaging_items',
                    JSON.stringify(values.PackagingItems || []),
                )

                // Add string fields for number_of_machines and total_capacity_value
                formData.append(
                    'number_of_machines',
                    values.number_of_machines || '',
                )
                formData.append(
                    'total_capacity_value',
                    values.total_capacity_value || '',
                )

                // Add date field
                formData.append(
                    'date_of_setting_up',
                    values.date_of_setting_up || '',
                )

                // Add waste management fields
                formData.append(
                    'total_waste_generated_value',
                    values.total_waste_generated_value || '',
                )
                formData.append(
                    'has_waste_storage_capacity',
                    values.has_waste_storage_capacity || '',
                )
                formData.append(
                    'waste_disposal_provision',
                    values.waste_disposal_provision || '',
                )
                ...
                if (licenseDetailProducer.id) {
                    const response = await AxiosBase.patch(
                        `/pmc/producers/${licenseDetailProducer.id}/`,
                        formData,
                        {
                            headers: {
                                'Content-Type': 'multipart/form-data',
                    ...
                    updateLicenseDetailProducer({
                        ...values,
                        id: response.data.id,
                    } as LicenseDetailFieldsProducer)
                } else {
                    const response = await AxiosBase.post(
                        '/pmc/producers/',
                        formData,
            ...
            if (completedSections.includes('licenseDetailConsumer')) {
                ...
                if (values.no_of_waste_disposible_bins) {
                    formData.append(
                        'no_of_waste_disposable_bins',
                        values.no_of_waste_disposible_bins.toString(),
                    )
                }
                formData.append(
                    'segregated_plastics_handed_over_to_registered_recyclers',
                    values.segregated_plastics_handed_over_to_registered_recyclers ||
                        'No',
                )
                formData.append('applicant', applicantDetail.id.toString())
                ...
                if (licenseDetailConsumer.id) {
                    ...
                    updateLicenseDetailConsumer({
                        ...values,
                        id: response.data.id,
                    } as LicenseDetailFieldsConsumer)
                } else {
                    const response = await AxiosBase.post(
                        '/pmc/consumers/',
                        formData,
            ...
            if (completedSections.includes('licenseDetailCollector')) {
                ...
                formData.append(
                    'total_capacity_value',
                    (values.total_capacity_value_collector || '').toString(),
                )
                formData.append(
                    'number_of_vehicles',
                    (values.number_of_vehicles || '').toString(),
                )
                formData.append(
                    'number_of_persons',
                    (values.number_of_persons || '').toString(),
                )
                ...
                if (licenseDetailCollector.id) {
                    ...
                    const response = await AxiosBase.patch(
                        `/pmc/collectors/${licenseDetailCollector.id}/`,
                        formData,
                        {
                            headers: {
                                'Content-Type': 'multipart/form-data',
                    ...
                    updateLicenseDetailCollector({
                        ...values,
                        id: response.data.id,
                    } as LicenseDetailFieldsCollector)
                } else {
                    const response = await AxiosBase.post(
                        '/pmc/collectors/',
                        formData,
            ...
            if (completedSections.includes('licenseDetailRecycler')) {
                ...
                if (licenseDetailRecycler.id) {
                    const response = await AxiosBase.patch(
                        `/pmc/recyclers/${licenseDetailRecycler.id}/`,
                        formData,
                        {
                            headers: {
                                'Content-Type': 'multipart/form-data',
                    ...
                    updateLicenseDetailRecycler({
                        ...values,
                        id: response.data.id,
                    } as LicenseDetailFieldsRecycler)
                } else {
                    const response = await AxiosBase.post(
                        '/pmc/recyclers/',
                        formData,
    ...
    const handleDocumentFormSubmit = async (
        ...
        if (!applicantDetail.id) {
            toast.push(
                <Notification type="danger">Applicant ID missing.</Notification>,
                { placement: 'top-center' },
            )
            return
        }

        const uploadDocument = async (file, description) => {
            const formData = new FormData()
    ...
    const submitApplication = async (values: LicenseDetailFormSchema) => {
        ...
        verifyChallanFormData.append(
            'ApplicantId',
            applicantDetail.id.toString(),
        ) // Assuming 'flow_diagram' is the challan image
            ...
            try {
                if (navigator.onLine) {
                    const response = await AxiosBase.get(
                        `/pmc/receipt-pdf?ApplicantId=${applicantDetail.id}`,
                        {
        ...
        <>
            ...
            {step === 4 && (
                <DocumentForm2
                    ...
                    defaultValues={LicenseDetailFormData}
                    readOnly={
                        applicantDetail.applicationStatus === 'Submitted' &&
                        isReadOnly2
                    }
            ...
            {step === 6 && (
                <ReviewAndSavePage onFormSubmit={submitApplication}>
                    <Container>
                        <div className="flex items-center justify-between px-8">
                            <span></span>
                            <div className="flex items-center">
