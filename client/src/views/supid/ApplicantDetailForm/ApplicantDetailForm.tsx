...
const validationSchema: ZodType<ApplicantDetailFormSchema> = z.object({
    ...
    phoneNumber: z
        .string()
        .regex(/^3\d{2}\d{7}$/, {
            message: 'Mobile number must be in the format 3999999999',
        })
        .refine((val) => val.length === 10, {
            message:
                'Mobile number must be complete and in the format 3999999999',
        }), // Ensure exact length
    // mobileOperator: z
    // .string()
    ...
    cnic: z
        .string()
        .regex(/^\d{5}-\d{7}-\d{1}$/, {
            message: 'CNIC must be in the format 99999-9999999-9',
        })
        .refine((val) => val.length === 15, {
            message: 'CNIC must be complete and in the format 99999-9999999-9',
        }), // Ensure exact length
