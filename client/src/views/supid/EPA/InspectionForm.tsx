...
const validationSchema = z
    .object({
        ...
        violationFound: z.string().nonempty({
            message: 'Please select at least one option for Violation Found',
        }),
        ...
        plasticBagsConfiscation: z.coerce.number().optional(),

        confiscation_otherPlastics: z
            .record(z.string(), z.coerce.number())
            .optional(),

        totalConfiscation: z.coerce.number().optional(),

        OtherSingleUseItems: z.array(z.string()).optional(),

        // ✅ Make `latitude` and `longitude` required
        latitude: z.coerce.number().min(-90, {
            message: 'Latitude is required and must be valid (-90 to 90)',
        }),
        longitude: z.coerce.number().min(-180, {
            message: 'Longitude is required and must be valid (-180 to 180)',
        }),
        ...
        inspectionDate: z
            .string()
            .min(1, { message: 'Inspection date is required' }), // Date field
        ...
        fineRecoveryBreakup: z
            .array(
                z.object({
                    date: z.string().min(1, { message: 'Date is required' }),
                    amount: z.coerce.number().min(0, {
                        message: 'Amount must be a number and at least 0',
                    }),
    ...
    .superRefine((data, ctx) => {
        if (data.fineRecoveryStatus === 'Recovered') {
            if (!data.paymentChallan) {
                ctx.addIssue({
                    path: ['paymentChallan'],
                    message:
                        'Payment challan is required when fine is recovered',
        ...
        if (isConfiscation) {
            ...
            if (
                !data.receiptBookNumber ||
                data.receiptBookNumber.trim() === ''
            ) {
                ctx.addIssue({
