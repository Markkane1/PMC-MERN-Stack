...
const violationTypeList = [
    {
        label: 'Plastic shopping/carry bags (having thickness less than 75 micron or less than 12 x 16 inch in size)',
        value: 'Plastic shopping/carry bags (having thickness less than 75 micron)',
    },
    { label: 'Other Single Use Plastics', value: 'Other Single Use Plastics' },
    {
        label: 'Unregistered Single Use Plastic Business',
        value: 'Unregistered Single Use Plastic Business',
    },
]
...
const actionTakenList = [
    ...
    { label: 'FIR', value: 'FIR' },
    {
        label: 'Complaint before Environmental Magistrate',
        value: 'Complaint before Environmental Magistrate',
    },
...
const InspectionDetailSection = ({
    ...
    const watchActionTaken = useWatch({
        control,
        name: 'actionTaken',
        defaultValue: [],
    })
    ...
    const watchFineAmount = useWatch({
        control,
        name: 'fineAmount',
        defaultValue: 0,
    })
    ...
    const plasticBagsConfiscation = useWatch({
        control,
        name: 'plasticBagsConfiscation',
        defaultValue: 0,
    })
    const otherPlasticsConfiscation = useWatch({
        control,
        name: 'confiscation_otherPlastics',
        defaultValue: {},
    })
    ...
    useEffect(() => {
        if (defaultValues?.fineRecoveryBreakup) {
            ...
            const loadedEntries = defaultValues.fineRecoveryBreakup.map(
                (entry) => ({
                    ...entry,
    ...
    const fineRecoveryBreakup = useWatch({
        control,
            ...
            const updatedEntries = prevEntries.map((entry, i) =>
                i === index
                    ? {
                          ...entry,
                          [key]:
                              key === 'amount' ? parseFloat(value) || 0 : value,
                      }
                    : entry,
            )
    ...
    return (
        <Card>
            ...
            <div className="grid md:grid-cols-2 gap-4">
                ...
                <FormItem
                    label="Plastic Application / License Number"
            ...
            {watchViolationFound === 'Yes' && (
                <div className="grid md:grid-cols-2 gap-4">
                    <FormItem
                        label="Type of Violation Found*"
                        invalid={Boolean(errors.violationType)}
                        errorMessage={errors.violationType?.message}
                    >
                    ...
                    <FormItem
                        label="Action Taken*"
                        invalid={Boolean(errors.actionTaken)}
                        errorMessage={errors.actionTaken?.message}
                    >
                                ...
                                render={({ field }) => (
                                    ...
                                    >
                                        <Checkbox
                                            ...
                                            disabled={
                                                readOnly &&
                                                action.value !== 'De Sealed'
                                            } // ✅ Disable all except "De Sealed"
                                        >
                                            {action.label}
                                        </Checkbox>
                                    </Checkbox.Group>
                    ...
                    {watchActionTaken.includes('Confiscation') &&
                        watchViolationType.includes(
                            ...
                            <FormItem
                                label="Plastic shopping/carry bags (having thickness less than 75 micron) (KG)"
                                invalid={Boolean(
                                    errors.plasticBagsConfiscation,
                                )}
                                errorMessage={
                                    errors.plasticBagsConfiscation?.message
                                }
                            >
                                <Controller
                                    name="plasticBagsConfiscation"
                    ...
                    {watchActionTaken.includes('Confiscation') && (
                        <>
                            <FormItem
                                ...
                                errorMessage={
                                    errors.confiscationReceipt?.message
                                }
                            >
                                <Controller
                    ...
                    {watchActionTaken.includes('Fine Imposed') && (
                        <>
                            ...
                            <FormItem
                                label="Fine Recovery Status"
                                invalid={Boolean(errors.fineRecoveryStatus)}
                                errorMessage={
                                    errors.fineRecoveryStatus?.message
                                }
                            >
                                <Controller
                                    name="fineRecoveryStatus"
                    ...
                    >
                        ...
                        />
                                    ...
                                    <FormItem
                                        label="Total Recovery Amount (PKR)"
                                        ...
                                        <Controller
                                            ...
                                            render={({ field }) => {
                                                useEffect(() => {
                                                    field.onChange(
                                                        totalRecovery,
                                                    ) // ✅ Update form state when totalConfiscation changes
