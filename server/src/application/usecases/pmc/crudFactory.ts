...
export function createCrudController(repo: CrudRepository, options?: {
  ...
  const list = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (options?.enablePagination && typeof (repo as any).listPaginated === 'function') {
      const { page, pageSize } = parsePaginationParams(req.query)
      const result = await (repo as any).listPaginated(query, page, pageSize)
      const data = options?.transform ? result.data.map((d: any) => options.transform?.(d, req)) : result.data
      return res.json(paginateResponse(data, result.pagination))
    }
